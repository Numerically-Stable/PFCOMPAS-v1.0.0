...
% quadrature - Generate integration points and weights for FEM
%
% This function returns an [n x 1] column vector W of quadrature weights 
% and an [n x dim] matrix of quadrature points Q. 
%
% PHASE-FIELD RELEVANCE:
% Standard linear/bilinear finite elements typically require only low-order 
% (e.g., 2x2) Gauss quadrature to exactly integrate the pristine elastic 
% stiffness matrix. However, phase-field fracture introduces a highly 
% non-linear degradation function g(phi) and steep spatial gradients 
% inside the crack bandwidth (l0). Having access to dynamically scalable, 
% high-order quadrature rules (up to order 16) is essential for researchers 
% testing higher-order elements or highly non-linear energy splits without 
% suffering from under-integration or volumetric locking.
%
% INPUT:
%   quadorder : Number of integration points per spatial dimension (1D)
%   qt        : Type of quadrature ('GAUSS' or 'TRIANGULAR')
%   sdim      : Spatial dimensions (1, 2, or 3)
%
% OUTPUT:
%   W : Quadrature weights
%   Q : Quadrature point coordinates in the reference domain
%
% Original base written by Jack Chessa (Northwestern University)
% Extended with high-order rules by Pranjal Saxena (IIT Kanpur)
  ...
  if ( strcmp(qt,'GAUSS') == 1 ) 
    ...
    % Pre-allocate memory based on the tensor-product expansion
    quadpoint=zeros(quadorder^sdim ,sdim);
    quadweight=zeros(quadorder^sdim,1);
  
    % 1D Reference Points (r1pt) and Weights (r1wt) defined on [-1, 1]
    ...
    % -------------------------------------------------------------------
    % TENSOR PRODUCT GENERATION FOR MULTI-DIMENSIONAL GAUSS QUADRATURE
    % 
    % 1D weights sum to 2.0. 
    % 2D weights sum to 4.0 (Area of reference square [-1, 1] x [-1, 1]).
    % 3D weights sum to 8.0 (Volume of reference cube).
    % -------------------------------------------------------------------
    if ( sdim == 1 ) 
      for i = 1:quadorder
        quadpoint(n,:) = [ r1pt(i) ];           
  ...
  elseif ( strcmp(qt,'TRIANGULAR') == 1 ) 
    ...
    if ( sdim == 3 )  %%% TETRAHEDRA
      ...
      elseif (quadorder == 5 )  %ps %  
          quadpoint = [ 0.250000000000000	0.250000000000000	0.250000000000000
                        ...
                        0.727272727272727	0.0909090909090909	0.0909090909090909
                        0.0909090909090909	0.0909090909090909	0.0909090909090909
                        0.0909090909090909	0.0909090909090909	0.727272727272727
                        0.0909090909090909	0.727272727272727	0.0909090909090909
          ...
          quadweight =  6.*[ 0.0302836780970892    ;
                          0.00602678571428572   ;
                          0.00602678571428572	;
                          0.00602678571428572   ;
                          0.00602678571428572	;
      ...
      Q=quadpoint;
      W=quadweight/6; % Normalizing to the volume of the reference tetrahedron (1/6)
      ...
      elseif (quadorder==8)
            % ps
            % Quadrature Points (2x14 array)
            quadpoint = [0.0694318442029737	0.0436530238707252
                                0.0694318442029737	0.214742881469342
                                0.0694318442029737	0.465284077898513
                                0.0694318442029737	0.715825274327684
                                0.0694318442029737	0.886915131926301
                                0.330009478207572	0.0465186775265609
                                0.330009478207572	0.221103222500738
                                0.330009478207572	0.448887299291690
                                0.330009478207572	0.623471844265867
                                0.669990521792428	0.0371926177849334
                                0.669990521792428	0.165004739103786
                                0.669990521792428	0.292816860422638
                                0.930568155797026	0.0146726751310273
                                0.930568155797026	0.0547591690719464];
        
            % Weights (1x14 array)
            quadweight = 2.*[0.0191734646470676
                                0.0387333412614463
                                0.0460377090452786
                                0.0387333412614463
                                0.0191734646470676
      ...
      % -------------------------------------------------------------------
      % % ATTENTION ATTENTION WHY DIVIDE TO 2?????
      % 
      % ANSWER: 
      % The raw quadrature weights from classical mathematical tables 
      % (such as those using barycentric/area coordinates L1, L2, L3) 
      % inherently sum to 1.0. 
      %
      % However, in finite element analysis, the integral is mapped over 
      % a standard reference right triangle defined by vertices (0,0), 
      % (1,0), and (0,1). The geometric area of this reference triangle 
      % is exactly 1/2. Therefore, to ensure that the sum of the numerical 
      % weights equals the area of the reference domain (Sum[W] = Area), 
      % the tabulated weights must be strictly divided by 2.
      % -------------------------------------------------------------------
      W=quadweight/2;   
    end
